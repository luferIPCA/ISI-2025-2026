...
namespace WS_Rest1.Controllers
{
    ...
    public class JWTAuthManager : IJWTAuthManager
    {
        ...
        {
            var token = GenerateJSONWebToken(username, DateTime.UtcNow, claims);
        ...
        {
            if (!AuthenticateUser(loginDetails)) return null;

            var token = GenerateJSONWebToken(loginDetails.Username, DateTime.UtcNow);
        ...
        /// <returns></returns>
        string GenerateJSONWebToken(string username, DateTime expires, Claim[] claims = null)
        {
        ...
        /// <returns></returns>
        private bool AuthenticateUser(AuthRequest loginDetalhes)
        {
            //Verifica na BD
