...
namespace RESTAuth.Controllers
{
    ...
    {
        /// <summary>
        /// Simula Bases de dados com users registados! Não está a ser usada!
        /// </summary>
        ...
        {
            //1. Get the user Role
            Roles role = ValidarUser(loginDetails); //metodo auxiliar
            if (role==Roles.Deny) return null;

            var cod = GetPolicy(loginDetails);  //metodo auxiliar

            // 2. Create the claims for "Roles"
            var claims = new[]
            {
                new Claim(ClaimTypes.Name, loginDetails.Username),
                new Claim(ClaimTypes.Role, role.ToString()),   // ⭐ IMPORTANT
                new Claim("cod", cod.ToString())               // a custom claim
            };

            var token = GenerateTokenString(loginDetails.Username, DateTime.UtcNow, claims);
        ...
        /// <returns></returns>
        //private User ValidarUser(AuthRequest loginDetalhes)
        private Roles ValidarUser(AuthRequest loginDetalhes)
        {
            //Verifica na BD o user
            //var user = users.SingleOrDefault(x => x.Username == model.Username && x.Password == model.Password);

            if (loginDetalhes.Username == "lufer" && loginDetalhes.Password == "Benfica2025")
            {
                return Roles.Admin;
            }

            if (loginDetalhes.Username == "lufer" && loginDetalhes.Password == "Porto2025")
            {
                return Roles.Reader;
            }
            return Roles.Deny;
        ...
        private string GetPolicy(AuthRequest loginDetails)
        {
            //Go to BD finding the Policy for this user
            if (loginDetails.Username == "lufer" && loginDetails.Password == "Benfica2025")
            {
                return "cod";       //simular
            }
            if (loginDetails.Username == "lufer" && loginDetails.Password == "Porto2025")
            {
                return "nocod";
            }
            return "";
